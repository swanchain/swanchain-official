<template>
  <div v-for="item in anchorProvider" :key="item.title" class="an-content" :id="item.targetId">
    <div v-if="item.mainTitleText" class="font-40 font-bold2 mt-32 mb-16 flex flex-ai-center">
      {{ item.mainTitleText }}
      <div v-if="item.externalLink.name" class="ex-link font-16 flex flex-ai-center ml-10 pointer" @click="openPage(item.externalLink.link)">
    ...
    <div v-else-if="item.title" class="font-28 font-bold2 mt-32 mb-16 flex flex-ai-center">
      {{ item.title }}
