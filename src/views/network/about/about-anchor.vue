    ...
    <div class="anchor">
      <div class="anchor-content w-100">
        <template v-for="item in anchor" :key="item.sourceId">
          <a :href="`#${item.targetId}`" @click="goLink(item)" :id="item.sourceId" :class="{ 'anchor-content-item font-16': true, active: item.actived && route.params.type === 'General-Information', 'main-title': item.mainTitle }">
            {{ item.title }}
          </a>
        </template>
        <template v-for="item in anchorProvider" :key="item.sourceId">
          <a :href="`#${item.targetId}`" @click="goLink(item)" :id="item.sourceId" :class="{ 'anchor-content-item font-16': true, active: item.actived && route.params.type === 'Providers', 'main-title': item.mainTitle }">
            {{ item.title }}
          </a>
        </template>
      </div>
